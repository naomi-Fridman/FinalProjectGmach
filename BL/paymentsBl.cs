...
namespace BL
{
    public class paymentsBl:IpaymentsBl
    {
        ...
        public paymentsBl(IPaymentDl _paymentsDl, ILoanDl _iLoandl, IWarningDl _iWarningDl, IpaymentTypeDl _IpaymentTypeDl,IUserDl _iuserDl, IMapper mapper)
        {
            iUserBl = iUserBl;
            iuserDl = _iuserDl;
        ...
        {
            return await iPaymentsDl.getAllPaymentsForLoan(userId,paymentId);
        }
        ...
        public async Task<int> postPayment(Payment payment)
        {
            ...
            paymentList = await getAllPaymentsForLoanByUserIdAndLoanDate(payment.UserId, l.LoanDate);//get all the payments for loan
            if(paymentList ==null)
            {
                payment.PaymentDate = (DateTime)l.FirstRepaymentDate;
            }
            else
            {
                paymentList.ForEach(p =>
                {
                    if (DateTime.Compare(p.PaymentDate, date) > 0)//get the last payment to know the date of the payment
                    {
                        date = p.PaymentDate;
                        lastPayment = p;
                    }
                ...
                payment.PaymentDate = lastPayment.PaymentDate.AddMonths(1);//add 1 month to the last payment
            }
            if (payment.PaymentDate<payment.InputDate)
            {
                payment.Comments+="התשלום נכנס באיחור ";
            }
            else if(payment.PaymentDate > payment.InputDate) 
            {
            ...
            if ((sum+payment.CollectionSum)==l.Sum)//the loan is finish
            {
                l.PaidUp = true;
                iLoanDl.updateLoan(l);
            }
            else if ((sum + payment.CollectionSum) > l.Sum)
            {
                payment.Comments += "שולם יותר ממה שהיה צריך יש זכות ללוה בסך:  " + (l.Sum - sum);
                l.PaidUp = true;
                iLoanDl.updateLoan(l);
            }
            return l.Sum - sum -payment.CollectionSum;
        }
        ...
        public async Task<List<DTO_User>> getAllPaymentsThatWerentPadeByDate(DateTime paymentDate)
        {
            ...
            dTO_LoansParams.MonthlyPaymentDay = paymentDate;
