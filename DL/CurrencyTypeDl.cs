...
namespace DL
{
    public class CurrencyTypeDl : ICurrencyTypeDl
    {
        gmach277Context gmachContext;

        public CurrencyTypeDl(gmach277Context _gmachContext)
        {
        ...
        {
            return await gmachContext.CurrencyType.ToListAsync();
        }
        public async Task<CurrencyType> getCurrencyById(int id)
        {
            return await gmachContext.CurrencyType.Where(x => x.Id == id).FirstOrDefaultAsync();
        }
        public API_Obj getExchangeRate()
        {
            try
            {
                String URLString = "https://v6.exchangerate-api.com/v6/a1dec4476936cf46c3745ea8/latest/USD";
