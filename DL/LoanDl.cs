...
namespace DL
{
    public class LoanDl : ILoanDl
    {
        ...
        public LoanDl(gmach277Context _gmachContext)
        {
        ...
        public async Task<List<Loan>> getAllLoans(DTO_LoansParams dTO_LoansParams)
        {
            ...
            var query = await gmachContext.Loan
                        ...
                        .Where(userAndLoan => (dTO_LoansParams.firstName == null || (userAndLoan.User.Id == userAndLoan.Loan.UserId && userAndLoan.User.FirstName == dTO_LoansParams.firstName))
                            ...
                            && (dTO_LoansParams.dateExact == null || DateTime.Compare(userAndLoan.Loan.LoanDate, dTO_LoansParams.dateExact == null ? DateTime.MinValue : dTO_LoansParams.dateExact.Value) == 0)
                            && (dTO_LoansParams.MonthlyPaymentDay == null || userAndLoan.Loan.MonthlyPaymentDay.Day == dTO_LoansParams.MonthlyPaymentDay.Value.Day)
                            && (dTO_LoansParams.dateFrom == null || DateTime.Compare(userAndLoan.Loan.LoanDate, dTO_LoansParams.dateFrom == null ? DateTime.MinValue : dTO_LoansParams.dateFrom.Value) >= 0)
                            && (dTO_LoansParams.dateTill == null || DateTime.Compare(userAndLoan.Loan.LoanDate, dTO_LoansParams.dateTill == null ? DateTime.MinValue : dTO_LoansParams.dateTill.Value) <= 0)).Select(l => l.Loan).ToListAsync();
            return query;
        }
        ...
        //    return l.Id;
        //}
        public Loan getLoanByUserId(int identityNumber)
        {
