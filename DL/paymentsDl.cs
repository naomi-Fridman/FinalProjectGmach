...
namespace DL
{
    ...
    public class paymentsDl : IPaymentDl
    {
        ...
        public paymentsDl(gmach277Context _gmachContext)
        {
        ...
        public async Task<List<Payment>> getAllPayments(DTO_PaymentParams dTO_PaymentParams)
        {
            var query = await gmachContext.Payment
             ...
             .Where(userAndPayment => (dTO_PaymentParams.firstName == null || (userAndPayment.User.Id == userAndPayment.Payment.UserId && userAndPayment.User.FirstName == dTO_PaymentParams.firstName))
                 ...
                 && (dTO_PaymentParams.CollectionSumFrom == null || userAndPayment.Payment.CollectionSum >= dTO_PaymentParams.CollectionSumFrom)
                 && (dTO_PaymentParams.CollectionSumTill == null || userAndPayment.Payment.CollectionSum <= dTO_PaymentParams.CollectionSumTill)
                 && (dTO_PaymentParams.CollectionSumExact == null || userAndPayment.Payment.CollectionSum == dTO_PaymentParams.CollectionSumExact)
                 && (dTO_PaymentParams.dateExact == null || DateTime.Compare(userAndPayment.Payment.PaymentDate, dTO_PaymentParams.dateExact == null ? DateTime.MinValue : dTO_PaymentParams.dateExact.Value) == 0)
                 && (dTO_PaymentParams.dateFrom == null || DateTime.Compare(userAndPayment.Payment.PaymentDate, dTO_PaymentParams.dateFrom == null ? DateTime.MinValue : dTO_PaymentParams.dateFrom.Value) >= 0)
                 && (dTO_PaymentParams.dateTill == null || DateTime.Compare(userAndPayment.Payment.PaymentDate, dTO_PaymentParams.dateTill == null ? DateTime.MinValue : dTO_PaymentParams.dateTill.Value) <= 0)).Select(p => p.Payment).ToListAsync();
            return query;
        ...
        {
            var p = gmachContext.Payment.Where(q => (q.CollectionSum > param.CollectionSumFrom || param.CollectionSumFrom == null));
            return await p.ToListAsync();
        }
        public async Task<List<Payment>> getAllPaymentsForLoan(int userId, int paymentId)
        {
            return await gmachContext.Payment.Where(d => d.UserId == userId && d.TypeId == paymentId).ToListAsync();
        }
        public async Task<List<Payment>> getAllPaymentsForLoanByUserIdAndLoanDate(int userId, DateTime loanDate)
        {
            var p = gmachContext.Payment.Where(p => (p.UserId == userId && DateTime.Compare(p.PaymentDate, loanDate) >= 0));
            return await p.ToListAsync();
        }
